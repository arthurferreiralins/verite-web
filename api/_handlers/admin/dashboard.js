...
module.exports = async function handler(req, res) {
  ...
  const [leadsResult, messagesResult, productsResult] = await Promise.all([
  ...
  const messages = { novo: 0, lido: 0, respondido: 0 };
  messagesResult.rows.forEach((r) => { messages[r.status] = r.n; });

  const products = { draft: 0, published: 0 };
  ...
  res.status(200).json({
    ...
    messages,
    messagesTotal: messages.novo + messages.lido + messages.respondido,
    products,
    productsTotal: products.draft + products.published,
  });
};
